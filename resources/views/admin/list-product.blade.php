      ...
      <div class="box-body table-responsive no-padding">
        <table class="table table-hover"><br>
          ...
          <tr>
            <td>{{ $no++ }}</td>
            <td>{{ $product->title }}</td>
            <td>{{ str_limit(strip_tags($product->desk, '100')) }}</td>
            <td><a href="{{ route('dashboard.edit-product', ['id' => $product->id ]) }}" class="btn btn-success"><i class="fa fa-edit"></i>  Edit</a>
            <button type="button" class="btn btn-danger hapus-produk" onclick="hapus_produk({{$product->id}})"><i class="fa fa-trash"></i> Hapus</button></td>
          </tr>
          @endforeach
        </table>
      </div>
          ...
          function(){
            setTimeout(function(){
              $.ajax({
                ...
                data : { product_id : window.productID, _token : "{{ csrf_token() }}"},
